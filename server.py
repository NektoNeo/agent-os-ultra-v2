...
import os, json, uuid, threading, subprocess, shlex, time, base64, tarfile, zipfile, re, io, hashlib, sqlite3, shutil
...
def ai_complete(req: AIReq, _=Depends(require_auth)):
    if req.provider == "openai":
        ...
        if not key: raise HTTPException(400, "OPENAI_API_KEY not set")
...
def root():
    return {"ok": True, "service": "Agent‑OS Ultra v2", "health": "/health"}


# === OPENAPI_PACK_TRUTH (robust) ===
from pathlib import Path as _Path
import json as _json, os as _os
from fastapi.responses import JSONResponse

_ROOT = _Path(__file__).resolve().parent  # абсолютная папка проекта (где server.py)

@app.get("/openapi/pack/{name}.json")
def openapi_pack_truth(name: str):
    # 1) пробуем статические файлы (истина)
    for base in ("openapi/pack", "actions/pack"):
        p = _ROOT / base / f"{name}.json"
        if p.exists():
            return JSONResponse(_json.loads(p.read_text(encoding="utf-8")))
    # 2) fallback: отдаём динамику, но проставляем флаг, чтобы Builder не спрашивал «разрешить?»
    spec = app.openapi()
...
# === OPS_MINIMAL_ENDPOINTS ===
import os, time, json
from datetime import datetime
from fastapi import Body, HTTPException

STATE_DIR = Path(os.getcwd()) / "workspace" / "_state"
STATE_DIR.mkdir(parents=True, exist_ok=True)

BRIEF_MD = STATE_DIR / "brief.md"
INDEX_JSON = STATE_DIR / "index.json"
TIMELINE_LOG = STATE_DIR / "timeline.log"   # jsonl
...
@app.get("/ops/brief")
def ops_brief():
    brief = BRIEF_MD.read_text(encoding="utf-8") if BRIEF_MD.exists() else "# Brief not created yet\\n"
    index = json.loads(INDEX_JSON.read_text(encoding="utf-8")) if INDEX_JSON.exists() else {}
    return {"ok": True, "brief_md": brief, "index": index, "paths": {"brief": str(BRIEF_MD), "index": str(INDEX_JSON), "timeline": str(TIMELINE_LOG)}}

@app.post("/ops/snapshot")
def ops_snapshot(payload: dict = Body(default=None)):
    # Мини-индекс проекта
    idx = {
        "ts": _now(),
        "project": "Agent‑OS Ultra v2",
        "packs": [],
        "domains": [],
        "last_jobs": [],
        "last_artifacts": [],
    ...
    # Обновим brief.md
    brief = payload.get("brief_md") if payload else None
    if not brief:
        brief = f"# Project Brief\\n\\nUpdated: {_now()}\\n\\nThis is an auto-generated snapshot.\\n"
    BRIEF_MD.write_text(brief, encoding="utf-8")
    INDEX_JSON.write_text(json.dumps(idx, ensure_ascii=False, indent=2), encoding="utf-8")
    return {"ok": True, "written": [str(BRIEF_MD), str(INDEX_JSON)]}

@app.post("/jobs/timeline/append")
def jobs_timeline_append(event: dict = Body(...)):
    event = {"ts": _now(), **event}
    with TIMELINE_LOG.open("a", encoding="utf-8") as f:
        f.write(json.dumps(event, ensure_ascii=False)+"\\n")
    return {"ok": True}

@app.get("/jobs/timeline/get")
def jobs_timeline_get(tail: int = 200):
    # Вернём [] даже если лог пуст/повреждён, никаких 500
    import json as _json
    items = []
    n = max(1, int(tail or 200))
        ...
        with TIMELINE_LOG.open("r", encoding="utf-8") as f:
            for ln in f.readlines()[-n:]:
                ...
                try:
                    ...
                    # пропускаем битые строки, не валимся
                    continue
    return {"ok": True, "items": items}

# Простая KV
KV_JSON = STATE_DIR / "kv.json"
def _kv_load():
    if KV_JSON.exists():
        return json.loads(KV_JSON.read_text(encoding="utf-8"))
    return {}
def _kv_save(d):
    KV_JSON.write_text(json.dumps(d, ensure_ascii=False, indent=2), encoding="utf-8")

@app.post("/kv/set")
def kv_set(kv: dict = Body(...)):
    d = _kv_load(); d.update(kv); _kv_save(d)
    return {"ok": True}

@app.get("/kv/get")
def kv_get(key: str):
    d = _kv_load()
    return {"ok": True, "value": d.get(key)}
